document.addEventListener("DOMContentLoaded", () => {
  ...
  function resizeHeliosCanvas() {
    const parent = canvas.parentElement;
    const availableWidth = parent ? parent.clientWidth : window.innerWidth;
    const mobile = window.innerWidth < 768;

    size = mobile
      ? Math.min(availableWidth, window.innerWidth - 28, 520)
      : Math.min(1000, Math.max(760, availableWidth || 1000));
  ...
  function drawFallbackBase() {
    ...
    const bgGlow = ctx.createRadialGradient(cx, cy, 40 * SCALE, cx, cy, 520 * SCALE);
    bgGlow.addColorStop(0, "rgba(0,220,255,0.08)");
    bgGlow.addColorStop(0.4, "rgba(80,120,255,0.04)");
    bgGlow.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = bgGlow;
    ctx.beginPath();
    ctx.arc(cx, cy, 520 * SCALE, 0, Math.PI * 2);
    ctx.fill();

    const outerDegRing = 470 * SCALE;
    const zodiacOuter = 390 * SCALE;
    const zodiacInner = 315 * SCALE;
    const innerField = 255 * SCALE;
    ...
    for (let i = 0; i < 360; i += 2) {
      ...
      let tickInner = outerDegRing - 8 * SCALE;
      if (i % 10 === 0) tickInner = outerDegRing - 16 * SCALE;
      if (i % 30 === 0) tickInner = outerDegRing - 28 * SCALE;
      ...
      ctx.lineWidth = i % 30 === 0 ? 2.4 * SCALE : 1 * SCALE;
      ctx.stroke();
    }
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const x = cx + Math.cos(r) * 350 * SCALE;
      const y = cy + Math.sin(r) * 350 * SCALE;
      ctx.font = `${Math.max(18, 54 * SCALE)}px Arial`;
      ctx.fillText(zodiac[i].glyph, x, y);
    }

    for (let i = 0; i < 360; i += 30) {
      const r = degToRad(i);
      const x = cx + Math.cos(r) * 515 * SCALE;
      const y = cy + Math.sin(r) * 515 * SCALE;
      ctx.font = `${Math.max(10, 24 * SCALE)}px Arial`;
  ...
  function drawHeliosCore() {
    const coreGlow = ctx.createRadialGradient(cx, cy, 15 * SCALE, cx, cy, 140 * SCALE);
    ...
    const colors = [
      "#ff2d55", "#ff7a00", "#ffd400", "#7dff00",
      "#00e5ff", "#0062ff", "#9b00ff", "#ff4fb3"
    ];
  ...
  function drawModelIfAvailable() {
    ...
    const overlayGlow = ctx.createRadialGradient(cx, cy, 10 * SCALE, cx, cy, 150 * SCALE);
  ...
  function drawCurvedFlow(longitude, color, index) {
    ...
    const startRadius = 120 * SCALE;
    const endRadius = 520 * SCALE;
    const bendRadius1 = (210 + (index % 4) * 10) * SCALE;
    const bendRadius2 = (340 + (index % 5) * 14) * SCALE;
  ...
  function drawPlanet(longitude, glyph, color, radius, name, index) {
    ...
    ctx.beginPath();
    ctx.arc(x - 6 * SCALE, y - 6 * SCALE, 6.5 * SCALE, 0, Math.PI * 2);
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(glyph, x, y + 1 * SCALE);

    let labelRadius = radius + 50 * SCALE;
    if (index % 3 === 1) labelRadius = radius + 72 * SCALE;
    if (index % 3 === 2) labelRadius = radius + 92 * SCALE;

    const lx = cx + Math.cos(r) * labelRadius;
    const ly = cy + Math.sin(r) * labelRadius;

    ctx.fillStyle = "#ffffff";
    ctx.font = `${Math.max(10, 19 * SCALE)}px Arial`;
    ctx.fillText(name, lx, ly);

    const signInfo = getSignInfo(longitude);
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    ctx.font = `${Math.max(9, 15 * SCALE)}px Arial`;
    ctx.fillText(
      `${signInfo.degreeInSign.toFixed(1)}° ${signInfo.sign}`,
      cx + Math.cos(r) * (labelRadius + 24 * SCALE),
      cy + Math.sin(r) * (labelRadius + 24 * SCALE)
    );
  }
  ...
  function drawChart(planets) {
    ...
    const innerAnchorRadius = (modelLoaded ? 380 : 392) * SCALE;
    const arcRadius = (modelLoaded ? 470 : 484) * SCALE;
    const planetRadius = (modelLoaded ? 560 : 574) * SCALE;

    planets.forEach((p, i) => {
      const lon = normalizeDeg(p.longitude);
      const color = p.color || "#ffffff";
      const stagger = (i % 2) * 14 * SCALE;
